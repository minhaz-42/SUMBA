...
After running `setup_initial_data --with-superuser`:
- **Username:** Set via `DJANGO_SUPERUSER_USERNAME` env var (default: `admin`)
- **Password:** Set via `DJANGO_SUPERUSER_PASSWORD` env var (default: `changeme`)

```bash
# Example: Create superuser with custom password
DJANGO_SUPERUSER_PASSWORD=your-secure-password python manage.py setup_initial_data --with-superuser
```
